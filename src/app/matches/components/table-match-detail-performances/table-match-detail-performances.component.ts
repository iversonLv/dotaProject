...
export class TableMatchDetailPerformancesComponent implements OnInit {
  ...
  getLargestData(data: any): any {
    ...
    this.teamsLargest.life_state_dead = Math.max(
      ...data.map((item) => item.life_state_dead)
    );
    this.teamsLargest.buyback_count = Math.max(
      ...data.map((item) => item.buyback_count)
    );
    this.teamsLargest.camps_stacked = Math.max(
      ...data.map((item) => item.camps_stacked)
    );
  ...
  getNotUndefinedData(data: any, field: string): number {
    ...
    const dataFilterNoSuchField = data.filter(
      (i) => Object.keys(i[field]).length !== 0
    );
    ...
    if (!data) {
      ...
      Object.keys(data).forEach((x) => {
        ...
        if (x.includes('track')) {
          ...
          obj.img =
            '/apps/dota2/images/abilities/alchemist_goblins_greed_md.png';
          obj[x] = data[x];
      ...
      if (trackArr.length > 0) {
        trackArr = trackArr.filter(
          (i) =>
            i.img === '/apps/dota2/images/abilities/bounty_hunter_track_md.png'
        );
