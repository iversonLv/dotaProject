...
export class ChartStatckLineComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.chartOption = {
      xAxis: {
        splitLine: {
          show: true,
          lineStyle: { color: 'rgba(184, 182, 180, .6)' },
        },
          ...
          params
            ...
            .forEach((i) => {
              ...
              <div class="tooltip-item-row ${
                i.data.content.isRadiant ? 'is-radiant' : ''
              }" style="color: white; border-color: ${i.color}">
                <div>
  ...
  calLegendData(data: any, playerColors: any, heroes: IheroLocal): string[] {
    ...
    data.map((i) => {
      ...
      legend.icon = `image://https://steamcdn-a.akamaihd.net/${
        heroes[i.hero_id].icon
      }`;
