...
export class VisionMapComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    // -4 which is border
    this.senPadding = [
      (this.mapSize * 45.9) / 900 - this.senObsSize[0] / 2 - 4,
      (this.mapSize * 45.9) / 900 - this.senObsSize[1] / 2 - 4,
    ];
    this.obsPadding = [
      (this.mapSize * 78.6) / 900 - this.senObsSize[0] / 2 - 4,
      (this.mapSize * 78.6) / 900 - this.senObsSize[1] / 2 - 4,
    ];
    // this.mapData = this.filterShowHideVisionDataFn(this.showHideVisionPlayersData, this.extractObsSenFinalData(this.data));
  }

  ngOnChanges(): void {
    this.mapData = this.filterShowHideVisionDataFn(
      this.showHideVisionPlayersData,
      this.extractObsSenFinalData(this.data)
    );
  }
  ...
  extractObsSenFinalData(data: any[]): any[] {
    const arr = [];
    data.forEach((i) => {
      const obsData = this.calObsSenData(
        data[0].duration,
        i.obs_left_log,
        i.obs_log,
        this.mapSize
      );
      const senData = this.calObsSenData(
        data[0].duration,
        i.sen_left_log,
        i.sen_log,
        this.mapSize
      );
      arr.push(...obsData, ...senData);
    });
    return arr;
  }
    ...
    placeDataKey.forEach((i) => {
      ...
      if (leftData[i]) {
        arr.push({
          ...leftData[i],
          ...d,
          lifespan: this.durationFormat.transform(
            leftData[i].time - placeData[i].time
          ),
  ...
  filterShowHideVisionDataFn(players: any, data: any[]): any[] {
    ...
    for (const i in players) {
      if (players.hasOwnProperty(i)) {
        arr.push(
          ...data.filter((x) => {
            const visionShowHideTimeline =
              this.visionTimeLine === -90
                ? true
                : x.time <= this.visionTimeLine &&
                  x.left_time_num >= this.visionTimeLine;

            // if the wards time and left time cover the vision timeline show it.
            if (players[i].obs_log && players[i].sen_log) {
              return (
                x.player_slot === players[i].player_slot &&
                (x.type === 'obs_log' || x.type === 'sen_log') &&
                visionShowHideTimeline
              );
